...
public class MondayTaskBoardService : ITaskBoardService
{
    ...
    public async Task<string> CreateFinanceTaskAsync(
        string groupTitle,
        string taskName,
        ...
        DateTime? deadline = null)
    {
        ...
        var boardId = await GetBoardIdByNameAsync("Stacy to do list")
                     ?? throw new InvalidOperationException("Stacy to do list board not found.");

        var groupId = await GetGroupIdByTitleAsync(boardId, groupTitle);

        string? userId = null;
        if (!string.IsNullOrWhiteSpace(assigneeEmail))
        {
            userId = await GetUserIdByEmailAsync(assigneeEmail);
        }
        ...
        {
            columnValues["status"] = new { label = statusLabel };
        }

        if (deadline.HasValue)
        {
            columnValues["date4"] = new { date = deadline.Value.ToString("yyyy-MM-dd") };
        }
        ...
        var columnValuesJson = JsonSerializer.Serialize(columnValues, new JsonSerializerOptions
        {
        ...
        using var response = await _mondayApiPolicy.ExecuteAsync(() => _httpClient.PostAsync(string.Empty, content));
        var body = await response.Content.ReadAsStringAsync();

        if (!response.IsSuccessStatusCode)
        {
            _logger.LogError(
                "Failed to create Stacy task. StatusCode: {StatusCode}, Body: {Body}",
                response.StatusCode,
                body);

            throw new InvalidOperationException($"Failed to create Stacy task. StatusCode: {(int)response.StatusCode}.");
        }

        var json = JsonNode.Parse(body);
        var itemId = json?["data"]?["create_item"]?["id"]?.ToString();

        if (string.IsNullOrWhiteSpace(itemId))
        {
            _logger.LogError("No item id returned when creating Stacy task. Body: {Body}", body);
            throw new InvalidOperationException("No item id returned when creating Stacy task.");
        }

        _logger.LogInformation(
            "Created Stacy task {ItemId} in group {GroupTitle} on board {BoardId}.",
            itemId,
            groupTitle,
            boardId);
    ...
    private async Task<string> GetGroupIdByTitleAsync(string boardId, string groupTitle)
    {
        var query = @"
        ...
        {
            _logger.LogError(
                "Failed to fetch groups for board {BoardId}. StatusCode: {StatusCode}, Body: {Body}",
                boardId,
                response.StatusCode,
                body);

            throw new InvalidOperationException($"Failed to fetch groups for board {boardId}.");
        }

        var json = JsonNode.Parse(body);
        var groups = json?["data"]?["boards"]?[0]?["groups"]?.AsArray();

        if (groups == null)
        {
            throw new InvalidOperationException($"No groups found on board {boardId}.");
        }

        foreach (var g in groups)
        {
            var title = g?["title"]?.ToString();
            if (!string.IsNullOrWhiteSpace(title) &&
                string.Equals(title, groupTitle, StringComparison.OrdinalIgnoreCase))
            {
        ...
        throw new InvalidOperationException(
            $"Group '{groupTitle}' not found on board {boardId}.");
    }
    ...
    public async Task<string> AttachMemberApplicationForm(string itemId, byte[] fileBytes, string fileName) =>
        await AttachFile(itemId, fileBytes, fileName, "file_mkq7bhzz");

    public async Task<string> AttachFinanceInvoiceFileAsync(string itemId, byte[] fileBytes, string fileName) =>
        await AttachFile(itemId, fileBytes, fileName, "file_mkq87vvq");

    private async Task<string> AttachFile(string itemId, byte[] fileBytes, string fileName, string columnId)
    {
    ...
    private async Task PostCreateBatchAsync(string boardId,List<(string ItemName, string ColumnValuesJson)> batch)
    {
        var sb = new StringBuilder();
        sb.Append("mutation(");
        sb.Append("$boardId: ID!,");
