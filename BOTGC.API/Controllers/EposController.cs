    ...
    IMediator mediator) : Controller
{
    ...
    public async Task<IActionResult> ExecuteTest([FromRoute] int memberId)
    {
        _logger.LogInformation("Executing test.");

        var query = new UpdateBenefitsAccountCommand(memberId, 2025, 150);
        var lines = await _mediator.Send(query, HttpContext.RequestAborted);
